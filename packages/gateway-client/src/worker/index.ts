...
import Client, { Address } from './pocket-client';
...
async function main() {
    ...
    const client = new Client(
        {
            host: process.env.NX_CADDY_HOST || self.location.hostname,
            port: parseInt(process.env.NX_CADDY_PORT || '4000'),
        },
        ({ lan, wan }: Address) => {
    ...
    const obj = (await localforage.getItem('addresses')) as Address;
    if (obj) {
    ...
    while (!(await localforage.getItem('welcome'))) {
        ...
        if (key && client) {
            ...
            client.navigate(
                `/@Ryan@pleroma.3b836fd1ff44e0fca4768822415ef6614d0c0d4f07b96008b1367734161e628.f.yg/posts/AMZQ43yUFFVkzIKKO0`
            );
        }
