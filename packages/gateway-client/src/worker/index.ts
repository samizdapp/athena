...
function patchFetchArgs(_reqObj: Request, _reqInit: RequestInit = {}) {
    ...
    if (url.pathname !== '/manifest.json') {
        _reqInit.headers.set('X-Intercepted-Subdomain', 'pleroma');
    }

    for (const pair of _reqObj.headers.entries()) {
        _reqInit.headers.set(pair[0], pair[1]);
...
async function main() {
    const bootstrapaddr =
        ...
        (await fetch('/pwa/libp2p.bootstrap')
            .then(r => r.text())
            .then(async id => {
